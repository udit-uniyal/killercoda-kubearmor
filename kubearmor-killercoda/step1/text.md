...
namespace="kubearmor" && duration=120 && (watch -n 1 "kubectl get po -n $namespace" & sleep $duration && while kubectl get po -n $namespace | awk 'NR > 1 {print $3}' | grep -q -v "Running"; do sleep 15; done; echo "All pods are in the 'Running' state. Exiting...") && kill $(jobs -p)
```{{exec}}
